...
__title__ = "make DIP switch 3D models exported to STEP and VRML"
...
from .switch_smd import make_case, make_pigs, make_pins_smd
...
def make_models(model_to_build=None, output_dir_prefix=None, enable_vrml=True):
    ...
    all_params = parameters.load_parameters("Switch_SMD")
    ...
    for model in models:
        ...
        pig_color = shaderColors.named_colors[
            all_params[model]["pig_color_key"]
        ].getDiffuseFloat()

        # Make the parts of the model
        body = make_case(all_params[model])
        pigs = make_pigs(all_params[model])

        CASE_THT_TYPE = "tht"
        CASE_SMD_TYPE = "smd"
        if all_params[model]["type"] == CASE_THT_TYPE:
            pins = make_pins_tht(all_params[model])

        if all_params[model]["type"] == CASE_SMD_TYPE:
            pins = make_pins_smd(all_params[model])

        # Put the parts in the correct orientation
        body = body.rotate((0, 0, 0), (0, 0, 1), all_params[model]["rotation"])
        pins = pins.rotate((0, 0, 0), (0, 0, 1), all_params[model]["rotation"])
        pigs = pigs.rotate((0, 0, 0), (0, 0, 1), all_params[model]["rotation"])
        ...
        component.add(
            pigs, color=cq_color_correct.Color(pig_color[0], pig_color[1], pig_color[2])
        )
