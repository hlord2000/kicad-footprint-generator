...
from .cq_connect_wago_734 import cqMakerWagoConn734
...
def make_models(model_to_build=None, output_dir_prefix=None, enable_vrml=True):
    ...
    all_params = parameters.load_parameters("Connector_Wago_734")
    ...
    for model in models:
        ...
        mod = cqMakerWagoConn734(all_params[model])
        body = mod.makePlasticCase()
        pins = mod.make_pins()
        body = body.translate(
            (mod.body_lenght / 2.0 - mod.body_thickness_x * 3.0, 0.0, 0.0)
        )
        pins = pins.translate(
            (mod.body_lenght / 2.0 - mod.body_thickness_x * 3.0, 0.0, 0.0)
        )
