...
__title__ = "make chip LED 3D models exported to STEP and VRML"
__author__ = (
    "scripts: Frank & Maurice & Stefan; models: see cq_model files; update: jmwright"
)
...
from .potentiometer_tht import make_chip
...
def make_models(model_to_build=None, output_dir_prefix=None, enable_vrml=True):
    ...
    all_params = parameters.load_parameters("Potentiometer_THT")
    ...
    for model in models:
        ...
        top_color = shaderColors.named_colors[
            all_params[model]["top_color_key"]
        ].getDiffuseFloat()
        pinmark_color = shaderColors.named_colors[
        ...
        if not package_found:
            print("Package {} not found, moving to next model.".format(model))
            continue

        # Apply rotation as needed to the parts
        ...
        component.add(
            ...
            color=cq_color_correct.Color(
                pinmark_color[0], pinmark_color[1], pinmark_color[2]
            ),
        )
