...
class GullwingGenerator(FootprintGenerator):
    ...
    def calc_pad_details(
        ...
        if Gmin_y - 2 * min_ep_to_pad_clearance < overrides["EP_y"]:
            heel_reduction = (
                overrides["EP_y"] + 2 * min_ep_to_pad_clearance - Gmin_y
            ) / 2
    ...
    def generateFootprint(
        self, device_params: dict[str, Any], pkg_id: str, header_info: dict[str, Any]
    ) -> None:
        gwc = GullwingConfiguration(
            device_params, header_info, pkg_id, self.configuration
        )
    ...
    def _createFootprintVariant(
        self,
        gwc: GullwingConfiguration,
        header: dict[str, Any],
        with_thermal_vias: bool,
    ) -> None:
        ...
        kicad_mod.tags = (
            ...
            .format(
                ...
                package=gwc.device_type,
                category=(
                    header["override_lib_name"]
                    if "override_lib_name" in header
                    else header["library_Suffix"]
                ),
            )
            .lstrip()
            .split()
