...
class PinHeaderGenerator(FootprintGenerator):
	...
	def generateFootprint(
		...
		for pos_count in fp_config.pos_range:
			...
			if (	fp_config.mount_type == "THT"
				...
				makePinHeadAngled(self, fp_config)
				pass
...
if __name__ == "__main__":
	...
	if not args.files:
		...
		args.files = list(Path(script_dir).rglob('pin_headers_*.yaml'))
		logging.info("No .yaml files given, using default .yaml file(s): {}".format(args.files))

	FootprintGenerator.run_on_files(PinHeaderGenerator, args)
