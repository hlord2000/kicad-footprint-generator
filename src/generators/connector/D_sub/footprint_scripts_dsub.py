...
def makeDSubStraight(
    ...
    description = "{0}-pin {1}, straight/vertical, THT-mount".format(
        pins, classname_description
    )
    ...
    for p in range(1, pinstop + 1):
        ...
        if p == 1:
            ...
            keepouts = keepouts + addKeepoutRect(
                x1, y1, pad + 8 * slk_offset, pad + 8 * slk_offset
            )
            x1pos = x1
        else:
            kicad_modg.append(
    ...
    allEqualSidedDownTriangle(
        kicad_modg,
        xcenter=[x1pos, -h_slk / 2 - text_size[0] * 0.75],
        side_length=text_size[0] / 2,
        layer="F.SilkS",
        width=lw_slk,
    )
...
def makeDSubEdge(
    ...
    description = "{0}-pin {1}, solder-cups edge-mounted".format(
        pins, classname_description
    )
    ...
    kicad_mod.append(
        PolygonLine(
            shape=[
                ...
                [
                    roundCrt(smaller_backcup_width / 2 + crt_offset),
                    roundCrt(ypcb_edge - crt_offset),
                ],
                [
                    roundCrt(smaller_backcup_width / 2 + crt_offset),
                    roundCrt(ypcb_edge + smaller_backcan_height - crt_offset),
                ],
                [
                    roundCrt(backcan_width / 2 + crt_offset),
                    roundCrt(ypcb_edge + smaller_backcan_height - crt_offset),
                ],
                [
                    roundCrt(backcan_width / 2 + crt_offset),
                    roundCrt(
                        ypcb_edge + smaller_backcan_height + backcan_height - crt_offset
                    ),
                ],
                [
                    roundCrt(shield_width / 2 + crt_offset),
                    roundCrt(
                        ypcb_edge + smaller_backcan_height + backcan_height - crt_offset
                    ),
                ],
                ...
                [
                    roundCrt(connwidth / 2 + crt_offset),
                    roundCrt(
                        ypcb_edge
                        + smaller_backcan_height
                        + backcan_height
        ...
        kicad_mod.append(
            PolygonLine(
                shape=[
                    ...
                    [
                        x_pin1 - topoffset - pad[0] / 2 - (slk_pad_offset + 2 * lw_slk),
                        y1 - pad[1] / 2 - (slk_pad_offset + 2 * lw_slk),
                    ],
                    [
                        x_pin1 - topoffset + rmx,
                        y1 - pad[1] / 2 - (slk_pad_offset + 2 * lw_slk),
                    ],
                ],
                layer=slk_layers_top,
    ...
    kicad_mod.append(
        Line(
            ...
            end=[shield_width / 2, pad[1] / 2 + soldercup_pad_edge_offset],
            layer="Dwgs.User",
            width=lw_crt,
        )
    )
    kicad_mod.append(
        Text(
            ...
            layer="Dwgs.User",
            size=[text_size[0] / 2, text_size[1] / 2],
            thickness=text_t / 2,
        )
    )
...
def makeDSubAngled(
    ...
    description = "{0}-pin {1}, horizontal/angled (90 deg), THT-mount".format(
        pins, classname_description
    )
    ...
    if hasMountingHoles:
        ...
        footprint_name = footprint_name + "_MountingHolesOffset{0:3.2f}mm".format(
            mounting_pcb_distance
        )
    else:
        if not hasNoBackBox:
    ...
    h_crt = (
        max([backbox_height, mounting_pcb_distance + mountingpad / 2])
        + max([nut_length, can_height])
        + shield_thickness
        + 2 * crt_offset
    )
    if hasNoBackBox:
    ...
    l_crt = -w_crt / 2
    t_crt = (
        -max([backbox_height - pin_pcb_distance - drmy, drmy + pad / 2]) - crt_offset
    )
    ...
    for p in range(1, pinstop + 1):
        ...
        if hasNoBackBox:
            kicad_modg.append(
                Line(
                    start=[x1 - lw_fab, y1],
                    end=[x1 - lw_fab, ypcb_edge - backcan_height],
                    layer="F.Fab",
                    width=lw_fab,
                )
            )
            kicad_modg.append(
                Line(
                    start=[x1, y1],
                    end=[x1, ypcb_edge - backcan_height],
                    layer="F.Fab",
                    width=lw_fab,
                )
            )
        ...
        kicad_modg.append(
            RectLine(
                start=[
                    ...
                    -mountingdistance / 2 + nut_diameter / 2,
                    ypcb_edge + shield_thickness + nut_length,
                ],
                layer="F.Fab",
                width=lw_fab,
            )
    ...
    if hasMountingHoles:
        kicad_modg.append(
            Line(
                start=[-mountingdistance / 2 - mountingdrill / 2, ypcb_edge],
                end=[
                    -mountingdistance / 2 - mountingdrill / 2,
                    ypcb_edge - mounting_pcb_distance,
                ],
                layer="F.Fab",
                width=lw_fab,
        ...
        kicad_modg.append(
            Arc(
                ...
                center=[-mountingdistance / 2, ypcb_edge - mounting_pcb_distance],
                angle=180,
                layer="F.Fab",
                width=lw_fab,
            )
    ...
    if not hasNoBackBox:
            ...
            PolygonLine(
                shape=[
                    ...
                    [
                        ...
                        ypcb_edge - backbox_height - slk_offset,
                    ],
                    [
                        backbox_width / 2 + slk_offset,
                        ypcb_edge - backbox_height - slk_offset,
        ...
        allEqualSidedDownTriangle(
            ...
            xcenter=[
                x1pos,
                ypcb_edge - backbox_height - slk_offset - text_size[0] * 0.75,
            ],
            side_length=text_size[0] / 2,
            layer="F.SilkS",
            width=lw_slk,
            ...
            PolygonLine(
                shape=[
                    ...
                    [
                        ...
                        ypcb_edge - backcan_height - slk_offset,
                    ],
                    [
                        leftmost - pad / 2 - slk_pad_offset,
                        ypcb_edge - backcan_height - slk_offset,
                    ],
                    [
                        leftmost - pad / 2 - slk_pad_offset,
                        ypcb_edge - back_height - slk_pad_offset,
                    ],
                    [
                        rightmost + pad / 2 + slk_pad_offset,
                        ypcb_edge - back_height - slk_pad_offset,
                    ],
                    [
                        rightmost + pad / 2 + slk_pad_offset,
        ...
        allEqualSidedDownTriangle(
            kicad_modg,
            xcenter=[x1pos, ypcb_edge - back_height - slk_offset - text_size[0] * 0.75],
            side_length=text_size[0] / 2,
            layer="F.SilkS",
            width=lw_slk,
        )
        ...
        kicad_mod.append(
            PolygonLine(
                shape=[
                    ...
                    [
                        roundCrt(offset[0] - can_width / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge + shield_thickness + crt_offset),
                    ],
                    [
                        roundCrt(offset[0] - shield_width / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge + shield_thickness + crt_offset),
                    ],
                    [
                        roundCrt(offset[0] - shield_width / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] - backcan_width / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] - backcan_width / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge - backcan_height - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] + leftmost - pad / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge - backcan_height - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] + leftmost - pad / 2 - crt_offset),
                        roundCrt(offset[1] + ypcb_edge - back_height - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] + rightmost + pad / 2 + crt_offset),
                        roundCrt(offset[1] + ypcb_edge - back_height - crt_offset),
                    ],
                    [
                        roundCrt(offset[0] + rightmost + pad / 2 + crt_offset),
