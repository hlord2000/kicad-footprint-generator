...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pincount, configuration):
    ...
    courtyard_x = round_to_grid(max(h_body_width + tab_width, (pad_x_spacing + pad_width)/2) + courtyard_clearance, courtyard_precision)
    courtyard_y = round_to_grid(h_body_length + courtyard_clearance, courtyard_precision)
    ...
    kicad_mod.append(PolygonLine(shape=fab_outline, layer='F.Fab', width=fab_width))
    ...
    kicad_mod.append(PolygonLine(shape=top_outline, layer='F.SilkS', width=silk_width))
