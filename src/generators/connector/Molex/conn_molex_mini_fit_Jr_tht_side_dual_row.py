...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins_per_row, variant, configuration):
    ...
    if peg:
        ...
        cxb_left = round_to_grid(body_edge['left'] - CrtYd_offset, CrtYd_grid)
        cxp_left = round_to_grid(-pad_size[0]/2 - CrtYd_offset, CrtYd_grid)

        cyb_bottom = round_to_grid(body_edge['bottom'] + CrtYd_offset, CrtYd_grid)
        cyb_mount_top = round_to_grid(body_edge['bottom'] -6.2 - CrtYd_offset, CrtYd_grid)
...
def generate_all(generator_name: str, global_config: GC.GlobalConfig, configuration: dict[str, Any]) -> int:
    ...
    for variant in variant_params:
        for pins_per_row in pins_per_row_range:
            generate_one_footprint(generator_name, global_config, pins_per_row, variant, configuration)
            num_fps_generated += 1
