...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins, params, configuration):
    ...
    kicad_mod.append(PolygonLine(shape=silk1, layer='F.SilkS', width=configuration['silk_line_width']))
    kicad_mod.append(PolygonLine(shape=silk1, layer='F.SilkS', width=configuration['silk_line_width'], y_mirror=P / 2))
    silk2 = [
        {'y': yt2-off, 'x': ret_dx+ret_size/2},
        {'y': yt2-off, 'x': xr1+off},
        {'y': P/2, 'x': xr1+off},
    ]
    kicad_mod.append(PolygonLine(shape=silk2, layer='F.SilkS', width=configuration['silk_line_width']))
    kicad_mod.append(PolygonLine(shape=silk2, layer='F.SilkS', width=configuration['silk_line_width'], y_mirror=P / 2))
    ...
    kicad_mod.append(PolygonLine(shape=tab, layer='F.SilkS', width=configuration['silk_line_width']))
    ...
    #     {'x': 0, 'y': 8},
    # ]
    # kicad_mod.append(PolygonLine(shape=pin1, layer='F.SilkS', width=0.12))
...
def generate_all(generator_name: str, global_config: GC.GlobalConfig, configuration: dict[str, Any]) -> int:
    ...
    for version in version_params:
        for pins_per_row in pins_per_row_range:
            generate_one_footprint(generator_name, global_config, pins_per_row, version_params[version], configuration)
            num_fps_generated += 1
