...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins, configuration):
    ...
    kicad_mod.append(PolygonLine(
        shape=[[round(x1, 2), round(y1, 2)], [round(x2, 2), round(y2, 2)]],
    ...
    if (pins > 3):
        for halfpins in range(int((pins / 2)) - 2):
            ...
            y2 = y2 + (2 * pitch)
            kicad_mod.append(PolygonLine(
                shape=[[round(x1, 2), round(y1 + 0.06, 2)],
    ...
    kicad_mod.append(PolygonLine(
        shape=[[round(xxx1, 2), round(yyy1, 2)], [round(xxx2, 2), round(yyy2, 2)]],
        layer='F.SilkS', width=configuration['silk_line_width']))
    xxx1 = xxx2
    yyy1 = yyy2
...
#        x2 = x1
#        y2 = round(y1 + 3, 0)
#        kicad_mod.append(PolygonLine(shape=[[round(x1, 2), round(y1, 2)],        [round(x2, 2),        round(y2, 2)]],        layer='F.SilkS', width=configuration['silk_line_width']))
