...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins, variant, configuration):
    ...
    if is_solder_mp:
        ...
        kicad_mod.append(
            Pad(
                ...
                fab_property=Pad.FabProperty.MECHANICAL,
                drill=mount_size_y,
    ...
    for i, Layer in enumerate(["F.Fab", "F.SilkS", "F.CrtYd"]):
        ...
        if Layer == "F.SilkS":  # SilkS
            kicad_mod.append(
                PolygonLine(shape=points, layer=Layer, width=LineWidth)
            )
            #
            ...
            kicad_mod.append(
                PolygonLine(shape=points2, layer=Layer, width=LineWidth)
            )
            #
            #
            points = []
