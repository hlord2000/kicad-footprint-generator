...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins, configuration):
    ...
    kicad_mod.append(PolygonLine(shape=side, width=configuration['silk_line_width'], layer='F.SilkS')) #,  layer='F.Fab'))
    kicad_mod.append(PolygonLine(shape=side, x_mirror=A / 2, width=configuration['silk_line_width'], layer='F.SilkS'))# ,layer='F.Fab'))
