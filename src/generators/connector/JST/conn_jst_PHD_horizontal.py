...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, pins, configuration):
    ...
    kicad_mod.append(PolygonLine(shape=side, width=configuration['fab_line_width'], layer='F.Fab'))
    ...
    if y1 < -size[1]/2.0:
        ...
        cy1 = round_to_grid(-size[1]/2.0-configuration['courtyard_offset']['connector'], configuration['courtyard_grid'])
    ...
    kicad_mod.append(PolygonLine(shape=side, x_mirror=A / 2.0, width=configuration['silk_line_width'], layer='F.SilkS'))
