...
def generate_one_footprint(generator_name: str, global_config: GC.GlobalConfig, params, model, configuration):
    ...
    if params.angled:
        ...
        if not params.flanged:
            ...
            kicad_mod.append(PolygonLine(shape=outline_poly, layer='F.SilkS', width=configuration['silk_line_width']))
            if configuration['with_fab_layer']:
                outline_poly=offset_polyline(outline_poly,-configuration['silk_fab_offset'],(center_x,0))
                kicad_mod.append(PolygonLine(shape=outline_poly, layer="F.Fab", width=configuration['fab_line_width']))
        ...
        for i in range(params.num_pins):
            ...
            plug_outline_translation.append(PolygonLine(shape=plug_outline_poly, layer='F.SilkS', width=configuration['silk_line_width']))
            ...
            if configuration['inner_details_on_fab']:
                plug_outline_translation.append(PolygonLine(shape=plug_outline_poly, layer="F.Fab", width=configuration['fab_line_width']))
    ...
    if not params.angled:
        ...
        kicad_mod.append(PolygonLine(shape=pin1_marker_poly, layer='F.SilkS', width=configuration['silk_line_width']))
        if configuration['with_fab_layer']:
            kicad_mod.append(PolygonLine(shape=pin1_marker_poly, layer='F.Fab', width=configuration['fab_line_width']))
    else:
        kicad_mod.append(PolygonLine(shape=create_pin1_marker_triangle(-params.pin_Sy / 2 - 0.2),
                                     layer='F.SilkS', width=configuration['silk_line_width']))
        if configuration['with_fab_layer']:
            kicad_mod.append(PolygonLine(
                shape=create_pin1_marker_triangle(bottom_y = 0,
                    dimensions = [params.pin_Sx - 0.2, -body_top_left[1]], with_top_line = False),
                layer='F.Fab', width=configuration['fab_line_width']))

    #################################################### 3d file ###################################################
    p3dname = '{prefix:s}{lib_name:s}.3dshapes/{fp_name}{suffix:s}'.format(
            prefix = configuration.get('3d_model_prefix', global_config.model_3d_prefix),
            suffix = configuration.get('3d_model_suffix', global_config.model_3d_suffix),
            lib_name=lib_name, fp_name=footprint_name)
